...
<div class="min-h-screen bg-slate-50 dark:bg-gray-900 flex flex-col items-center justify-center p-4 transition-colors duration-500">
  ...
  <div class="text-center max-w-2xl bg-white dark:bg-gray-800 p-10 rounded-3xl shadow-2xl dark:shadow-purple-900/20 shadow-indigo-100 border border-gray-100 dark:border-gray-700 transition-all duration-500">
    <h1 class="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-fuchsia-600 dark:from-purple-400 dark:to-pink-600 mb-6 drop-shadow-sm">
      {{ state.activeLanguage() === 'en' ? 'Discover your true adventurer career' : 'ค้นหาอาชีพ นักผจญภัย ที่แท้จริงของคุณ' }}
    </h1>
    ...
    >
      <span>⚔️</span> {{ state.activeLanguage() === 'en' ? 'Start Adventure' : 'เริ่มการผจญภัย' }}
    </button>
  </div>
</div>
  `
