...
@Component({
  ...
  template: `
    <div
      class="min-h-screen bg-slate-50 dark:bg-gray-900 flex flex-col items-center justify-center p-4 pt-24 lg:p-8 transition-colors duration-500 relative"
    >
      <app-language-toggle></app-language-toggle>

      <div
        class="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-3xl shadow-2xl dark:shadow-purple-900/20 shadow-indigo-100 p-8 border border-gray-100 dark:border-gray-700 transition-all duration-500"
      >
        <!-- Progress Bar -->
        <div class="mb-10">
          <div
            class="flex justify-between text-slate-500 dark:text-gray-400 text-sm mb-3 font-bold uppercase tracking-wider"
          >
            <span
              >{{ state.activeLanguage() === 'en' ? 'Question' : 'คำถามที่' }}
              {{ state.currentQuestionIndex() + 1 }}</span
            >
            <span>{{ state.currentQuestionIndex() + 1 }} / {{ totalQuestions }}</span>
          </div>
          <div
            class="w-full bg-slate-200 dark:bg-gray-700 rounded-full h-4 overflow-hidden shadow-inner"
          >
            <div
        ...
        @if (currentQuestion()) {
          <!-- Question Text -->
          <h2
            class="text-xl md:text-2xl font-extrabold text-slate-800 dark:text-white mb-10 text-center leading-snug drop-shadow-sm transition-colors duration-500"
          >
