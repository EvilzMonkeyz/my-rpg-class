...
@Component({
  ...
  template: `
    <div
      class="min-h-screen bg-slate-50 dark:bg-gray-900 text-slate-800 dark:text-gray-100 p-4 pt-24 md:p-8 flex flex-col items-center transition-colors duration-500 relative"
    >
      <app-language-toggle></app-language-toggle>

      @if (determinedClass && stats) {
        <div
          class="max-w-4xl w-full flex flex-col items-center space-y-8 animate-fade-in-up mt-8 md:mt-12"
        >
          <!-- Header info -->
          <div
            class="text-center w-full bg-white dark:bg-gray-800 p-10 rounded-3xl shadow-xl dark:shadow-purple-900/20 shadow-indigo-100 border border-slate-100 dark:border-gray-700 transition-all duration-500 relative overflow-hidden"
          >
            <!-- Decorative background blob -->
            <div
              class="absolute -top-16 -right-16 w-32 h-32 bg-indigo-50 dark:bg-gray-700/50 rounded-full blur-2xl -z-10"
            ></div>

            <h1
              class="text-5xl md:text-6xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-amber-500 to-rose-500 dark:from-yellow-400 dark:to-red-500 uppercase drop-shadow-sm mb-6"
            >
            ...
            @if (copySuccess) {
              <p class="text-emerald-500 font-bold mt-4 animate-pulse">
                {{ lang === 'en' ? 'Link Copied!' : 'คัดลอกลิงก์แล้ว!' }}
              </p>
            }
          </div>
          ...
          <!-- Radar Chart Section -->
          <div
            class="w-full md:w-2/3 bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-gray-700 aspect-square md:aspect-auto transition-all duration-500"
          >
            <h2
              class="text-2xl font-extrabold text-center mb-6 text-indigo-500 dark:text-purple-400 capitalize tracking-wide"
            >
              {{ lang === 'en' ? 'Your Core Attributes' : 'ค่าสถานะหลักของคุณ' }}
            </h2>
          ...
          <div
            class="w-full bg-slate-900 border border-slate-800 dark:bg-gradient-to-br dark:from-gray-800 dark:to-gray-900 px-10 sm:px-12 py-12 sm:py-16 rounded-3xl shadow-2xl text-center relative overflow-hidden text-white transition-all duration-500"
          >
            <!-- Decorative badge -->
            <div
              class="absolute top-0 right-0 bg-yellow-500 text-slate-900 font-extrabold px-6 py-2 rounded-bl-2xl shadow-lg border-b border-l border-yellow-400"
            >
              {{ lang === 'en' ? 'SPONSORED' : 'ผู้สนับสนุน' }}
            </div>

            <h2
              class="text-xl sm:text-2xl flex items-center justify-center  font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400 mb-6 drop-shadow"
            >
            ...
            >
              🏷️ {{ lang === 'en' ? 'Upgrade' : 'อัปเกรด' }}
              {{ determinedClass.gearRecommendation[lang].name }}
              {{ lang === 'en' ? 'now!' : 'เลย!' }}
            </a>
          </div>
          ...
          <button
            (click)="retakeQuiz()"
            class="mt-4 text-slate-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-white underline font-bold transition flex items-center"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
